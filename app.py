...
# ==================== NEW ROUTES FOR CV PROCESSING ====================

@app.route('/upload-cv', methods=['GET', 'POST'])
def upload_cv():
    """Upload và xử lý CV từ file PDF/Image"""
    if request.method == 'POST':
        # Kiểm tra file upload
        if 'cv_file' not in request.files:
            flash('❌ Không tìm thấy file CV. Vui lòng chọn file PDF hoặc ảnh.', 'error')
            return redirect(request.url)
        ...
        if file.filename == '':
            flash('❌ Chưa chọn file CV. Vui lòng chọn file PDF hoặc ảnh chứa thông tin ứng viên.', 'error')
            return redirect(request.url)
        ...
        if file and any(file.filename.lower().endswith(ext) for ext in allowed_extensions):
            try:
                ...
                print(f"📄 CV đã lưu: {filepath}")

                # Xử lý CV và dự đoán
                print("🎯 Bắt đầu xử lý CV...")
                result = hr_system.predict_from_cv(filepath, position_applied)

                if result['success']:
                    # Thông báo thành công
                    success_msg = f"🎉 Xử lý CV thành công cho ứng viên {result['candidate_data']['candidate_id']}: {result['prediction']['prediction_vietnamese']}"
                    flash(success_msg, 'success')
                    ...
                    # Check if this is an AJAX request from predict page
                    if request.headers.get('X-Requested-With') == 'XMLHttpRequest' or 'application/json' in request.headers.get('Content-Type', ''):
                        return jsonify({
                            'success': True,
                            'redirect': url_for('cv_results'),
                            'candidate_id': result['candidate_data']['candidate_id'],
                            'prediction': result['prediction']['prediction_vietnamese'],
                    ...
                    error_msg = f'❌ Lỗi xử lý CV: {result["error"]}'
                    flash(error_msg, 'error')
                ...
                error_msg = f'❌ Lỗi xử lý file CV: {str(e)}'
                flash(error_msg, 'error')
            ...
            flash('❌ Chỉ chấp nhận file PDF và các định dạng ảnh (JPG, PNG, BMP, TIFF)', 'error')

    return render_template('upload_cv.html')

@app.route('/improvement-plan', methods=['GET', 'POST'])
def improvement_plan():
    """Tạo kế hoạch phát triển kỹ năng cá nhân hóa"""
    if request.method == 'POST':
        try:
            # Lấy dữ liệu từ form
            candidate_data = {
                ...
                'education_level': request.form.get('education_level', ''),
                'skills': request.form.get('skills', '').strip(),
                'position_applied': request.form.get('target_position', '').strip(),
                'projects': request.form.get('projects', '').strip().split('\n') if request.form.get('projects') else [],
                'certifications': request.form.get('certifications', '').strip().split('\n') if request.form.get('certifications') else []
            }

            # Validate dữ liệu
            if not candidate_data['candidate_id']:
                flash('❌ Vui lòng nhập Mã ứng viên', 'error')
                return render_template('improvement_plan.html', candidate=candidate_data)

            if not candidate_data['skills']:
                flash('❌ Vui lòng nhập danh sách kỹ năng', 'error')
                return render_template('improvement_plan.html', candidate=candidate_data)

            if not candidate_data['position_applied']:
                flash('❌ Vui lòng chọn vị trí mục tiêu', 'error')
                return render_template('improvement_plan.html', candidate=candidate_data)

            # Tạo kế hoạch phát triển
            print(f"📈 Tạo kế hoạch phát triển cho {candidate_data['candidate_id']}")
            improvement_plan = hr_system.generate_improvement_plan(candidate_data, candidate_data['position_applied'])

            if 'error' in improvement_plan:
                flash(f'❌ Lỗi tạo kế hoạch: {improvement_plan["error"]}', 'error')
            else:
                success_msg = f"✅ Tạo kế hoạch phát triển thành công cho vị trí {candidate_data['position_applied']}"
                flash(success_msg, 'success')
            ...
            error_msg = f'❌ Tạo kế hoạch thất bại: {str(e)}'
            flash(error_msg, 'error')
...
def page_not_found(e):
    ...
    return render_template('error.html',
                         error_code='404',
                         error_title='Trang không tìm thấy',
                         error_description='Rất tiếc, trang bạn đang tìm kiếm không tồn tại hoặc đã bị di chuyển.'), 404
...
def internal_error(e):
    ...
    return render_template('error.html',
                         ...
                         error_description='Xin lỗi, đã có lỗi xảy ra ở phía server. Chúng tôi đang khắc phục vấn đề này.'), 500
