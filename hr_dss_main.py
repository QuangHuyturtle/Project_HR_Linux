...
# -*- coding: utf-8 -*-
"""
HR Decision Support System - Enhanced Version with Required Skills Logic & CV Processing
Hệ thống hỗ trợ ra quyết định tuyển dụng nhân sự - Phiên bản nâng cao với logic kỹ năng bắt buộc và xử lý CV
...
class HRDecisionSupportSystemEnhanced:
    ...
    def __init__(self, model_path="models/", data_path="data/", upload_path="uploads/"):
        """
        ...
        Args:
            ...
            upload_path (str): Đường dẫn upload file
        """
        ...
        self.cv_processor = CVProcessor(str(self.upload_path))
        ...
        logger.info("🚀 Hệ thống Hỗ trợ Ra quyết định Tuyển dụng (Enhanced with CV Processing) đã được khởi tạo")
    ...
    # ==================== NEW METHODS FOR CV PROCESSING ====================

    def process_cv_file(self, cv_file_path: str) -> dict:
        """
        Xử lý file CV (PDF/Image) và trích xuất thông tin

        Args:
            cv_file_path (str): Đường dẫn đến file CV
        ...
        try:
            ...
            # Process CV using CV Processor
            cv_data = self.cv_processor.process_cv(cv_file_path)

            # Save processing result
            result_file = self.cv_processor.save_processing_result(cv_data)

            logger.info(f"✅ CV processing completed: {cv_file_path}")
            return {
                'success': True,
                'cv_data': cv_data,
    ...
    def predict_from_cv(self, cv_file_path: str, position_applied: str = None) -> dict:
        """
        Dự đoán độ phù hợp từ file CV

        Args:
            cv_file_path (str): Đường dẫn đến file CV
            position_applied (str): Vị trí ứng tuyển (nếu có trong CV)

        Returns:
            dict: Kết quả dự đoán và gợi ý
        """
        try:
            # Process CV
            cv_result = self.process_cv_file(cv_file_path)
            ...
            candidate_data = self._extract_candidate_data_from_cv(cv_data, position_applied)

            # Perform prediction
            prediction_result = self.predict_candidate(candidate_data)

            # Generate recommendations if not suitable
            recommendations = None
            if not prediction_result.get('suitable', False):
                recommendations = self.recommendation_engine.analyze_skill_gap(
                    candidate_data,
                    position_applied or candidate_data.get('position_applied', 'general')
                )
            ...
            logger.error(f"❌ Error predicting from CV: {str(e)}")
            return {
                'success': False,
                'error': str(e),
                'prediction': None,
    ...
    def generate_improvement_plan(self, candidate_data: dict, target_position: str) -> dict:
        """
        Tạo kế hoạch phát triển kỹ năng chi tiết

        Args:
            candidate_data (dict): Dữ liệu ứng viên
            target_position (str): Vị trí mục tiêu

        Returns:
            dict: Kế hoạch phát triển
        """
        try:
            logger.info(f"📈 Generating improvement plan for position: {target_position}")

            # Analyze skill gap
            skill_gap_analysis = self.recommendation_engine.analyze_skill_gap(
                candidate_data, target_position
            )

            # Generate report
            report = self.recommendation_engine.generate_report(skill_gap_analysis)

            # Create actionable plan
            improvement_plan = {
                'skill_gap_analysis': skill_gap_analysis,
                'text_report': report,
                'next_steps': self._create_next_steps(skill_gap_analysis),
                'estimated_cost': self._estimate_learning_cost(skill_gap_analysis),
                'success_metrics': self._define_success_metrics(skill_gap_analysis)
            }
    ...
    def _extract_candidate_data_from_cv(self, cv_data: dict, position_applied: str = None) -> dict:
        """
        Trích xuất dữ liệu ứng viên từ kết quả xử lý CV

        Args:
            cv_data (dict): Dữ liệu từ CV processing
            position_applied (str): Vị trí ứng tuyển

        Returns:
            dict: Dữ liệu ứng viên chuẩn hóa
        """
        # Extract skills
        all_skills = cv_data.get('all_skills', [])
        if not all_skills and cv_data.get('skills_by_category'):
            all_skills = []
            for category_skills in cv_data['skills_by_category'].values():
        ...
        education_level = cv_data.get('highest_degree', 'high_school')
        education_level = education_mapping.get(education_level.lower(), 'high_school')

        # Extract experience
        years_experience = cv_data.get('total_years', 0)
        if years_experience == 0 and cv_data.get('companies'):
            # Estimate from number of companies
            years_experience = min(len(cv_data['companies']) * 2, 20)

        # Create candidate profile
        candidate_data = {
            'candidate_id': cv_data.get('file_name', '').replace('.pdf', '').replace('.jpg', ''),
            'years_experience': years_experience,
            'education_level': education_level,
            'skills': skills_str,
            'experience_description': self._generate_experience_description(cv_data),
            'position_applied': position_applied or self._infer_position_from_skills(all_skills),
    ...
    def _generate_experience_description(self, cv_data: dict) -> str:
        """
        Tạo mô tả kinh nghiệm từ dữ liệu CV
        """
        parts = []

        # Experience years
        years = cv_data.get('total_years', 0)
        if years > 0:
        ...
        if companies:
            if len(companies) == 1:
                parts.append(f"Worked at {companies[0]}")
            elif len(companies) <= 3:
                parts.append(f"Worked at companies including {', '.join(companies)}")
            else:
                parts.append(f"Worked at multiple companies including {', '.join(companies[:3])}")

        # Skills highlight
        skills = cv_data.get('all_skills', [])[:5]  # Top 5 skills
        if skills:
            parts.append(f"Skilled in {', '.join(skills)}")

        # Projects
        project_count = cv_data.get('project_count', 0)
        if project_count > 0:
            parts.append(f"Completed {project_count} projects")

        # Education
        education = cv_data.get('highest_degree')
        if education:
            parts.append(f"Education: {education}")

        return '. '.join(parts) if parts else 'Professional with relevant experience'

    def _infer_position_from_skills(self, skills: list) -> str:
        """
        Suy luận vị trí phù hợp từ kỹ năng
        """
        skills_lower = [skill.lower() for skill in skills]

        # Count skill matches for each position
        position_scores = {}

        for position, requirements in self.required_skills_by_position.items():
            score = 0
            required_skills = requirements.get('required', [])
            preferred_skills = requirements.get('preferred', [])

            for skill in required_skills:
                if any(req in skill.lower() for req in skills_lower):
                    score += 2

            for skill in preferred_skills:
                if any(pref in skill.lower() for pref in skills_lower):
                    score += 1
        ...
        if position_scores:
            ...
            if position_scores[best_position] > 0:
                return best_position.replace('_', ' ')

        return 'general'

    def _create_next_steps(self, skill_gap_analysis: dict) -> list:
        """
        Tạo danh sách các bước tiếp theo cụ thể
        """
        next_steps = []
        recommendations = skill_gap_analysis.get('recommendations', {})

        # Immediate priorities
        immediate = recommendations.get('immediate_priorities', [])
        for item in immediate[:3]:  # Top 3 priorities
            next_steps.append({
                'action': f"Learn {item['skill']}",
                'priority': 'High',
                'estimated_time': item.get('estimated_time', '4 weeks'),
                'type': 'Skill Development'
            })

        # Add certifications
        certs = recommendations.get('skill_certifications', [])
        for cert in certs[:2]:  # Top 2 certifications
            next_steps.append({
                'action': f"Get {cert['name']} certification",
                'priority': 'Medium',
    ...
    def _estimate_learning_cost(self, skill_gap_analysis: dict) -> dict:
        """
        Ước tính chi phí học tập
        """
        costs = {
            'courses': 0,
            'certifications': 0,
        ...
        learning_resources = skill_gap_analysis.get('recommendations', {}).get('learning_resources', {})
        course_count = sum(len(resources.get('courses', [])) for resources in learning_resources.values())
        costs['courses'] = course_count * 50  # Average $50 per course

        # Estimate certification costs
        certs = skill_gap_analysis.get('recommendations', {}).get('skill_certifications', [])
        costs['certifications'] = len(certs) * 150  # Average $150 per certification

        # Estimate book costs
        book_count = sum(len(resources.get('books', [])) for resources in learning_resources.values())
        costs['books'] = book_count * 30  # Average $30 per book

        costs['total_estimated'] = costs['courses'] + costs['certifications'] + costs['books']

        return costs

    def _define_success_metrics(self, skill_gap_analysis: dict) -> list:
        """
        Định nghĩa các metrics đo lường thành công
        """
        metrics = []

        gap_analysis = skill_gap_analysis.get('skill_gap_analysis', {})

        # Skills completion metrics
        missing_critical = len(gap_analysis.get('missing_required_skills', []))
        missing_advanced = len(gap_analysis.get('missing_advanced_skills', []))

        if missing_critical > 0:
            metrics.append(f"Complete {missing_critical} critical skills within 3 months")

        if missing_advanced > 0:
            metrics.append(f"Master {missing_advanced} advanced skills within 6 months")

        # Certification metrics
        certs = skill_gap_analysis.get('recommendations', {}).get('skill_certifications', [])
        if certs:
            metrics.append(f"Obtain {len(certs)} relevant certifications")

        # Project metrics
        metrics.append("Complete 2-3 portfolio projects demonstrating new skills")
        metrics.append("Achieve 80% skill completion for target position")

        # Overall readiness
        current_score = skill_gap_analysis.get('overall_score', {}).get('overall_score', 0)
        target_score = max(80, current_score + 20)
        metrics.append(f"Improve readiness score from {current_score}% to {target_score}%")
